...
$Id: PfamScan.pm,v 1.1 2016/08/03 21:23:27 jig2114 Exp $
...
sub search {
  ...
  if ( keys %AllResults ) {
    ...
    foreach my $AllResult ( sort keys %AllResults ) {

      foreach my $seq_id ( sort keys %{ $self->{_seq_hash} } ) {
          ...
          if ( $result->seqName eq $seq_id ) {
            ...
            foreach my $result2 ( @{ $AllResults{$AllResult} } ) {
              ...
              if ( $result2->seqName eq $seq_id ) {
                foreach my $hmmname ( sort keys %{ $result2->seqs } ) {
