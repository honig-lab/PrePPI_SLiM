...
Usage:
    python PrP_LR_job.py -batch <batch_directory> -g <genome> -b <BN_file> -o <output_file> [-v] [-d]
...
def process_tar_file(args):
            ...
            for member in tar.getmembers():
                if (member.name.endswith("ProtPeptide_ELM.txt") and
                    ...
                    for idx, row in df.iterrows():
                        ...
                        if pp1 not in lrs or lrs[pp1][0] < lr_value:
                            lrs[pp1] = (lr_value, cls) # Store both LR and ELM class
    except Exception as e:
    ...
    try:
        with open(output_file, 'w') as out_f:
            out_f.write("# record_type=PrePPI-SLiM_likelihood_ratios\n")
